...
def test_supply_chain_risk_agent_contract() -> None:
    ...
    supply_chain_agent = next(
        element for element in architecture["elements"] if element["name"] == "SupplyChainRiskAgent"
    )
    assert supply_chain_agent["browser_path"].endswith("/SupplyChainRiskAgent")
    assert supply_chain_agent["attributes"][0]["description"] == (
