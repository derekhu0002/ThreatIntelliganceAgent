...
def test_incident_reporting_agent_contract() -> None:
    ...
    assert "Boundary Notes" in skill_prompt
    assert "Empty Result Contract" in skill_prompt
    assert '"IncidentReportingAgent"' in ai4x_tool

    incident_reporting_agent = next(
        element for element in architecture["elements"] if element["name"] == "IncidentReportingAgent"
    )
    assert incident_reporting_agent["browser_path"].endswith("/IncidentReportingAgent")
    assert incident_reporting_agent["attributes"][0]["description"] == (
        "agent_app\\opencode_app\\.opencode\\agents\\IncidentReportingAgent.md"
    )
    assert incident_reporting_agent["attributes"][1]["description"] == (
        "agent_app\\opencode_app\\.opencode\\skills\\incident_report_generation\\SKILL.md"
    )

    agents_view = next(view for view in architecture["views"] if view["view_name"] == "Agents")
    assert incident_reporting_agent["id"] in agents_view["included_elements"]
