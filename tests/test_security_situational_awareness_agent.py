...
def test_security_situational_awareness_agent_contract() -> None:
    ...
    assert "name: security-posture-window-summary" in skill_prompt
    ...
    assert "Top Risk Drivers" in skill_prompt
    ...
    awareness_agent = next(
        element for element in architecture["elements"] if element["name"] == "SecuritySituationalAwarenessAgent"
    )
    assert awareness_agent["browser_path"].endswith("/SecuritySituationalAwarenessAgent")
    assert awareness_agent["attributes"][0]["description"] == (
        "agent_app\\opencode_app\\.opencode\\agents\\SecuritySituationalAwarenessAgent.md"
    )
    assert awareness_agent["attributes"][1]["description"] == (
        "agent_app\\opencode_app\\.opencode\\skills\\security-posture-window-summary\\SKILL.md"
    )
