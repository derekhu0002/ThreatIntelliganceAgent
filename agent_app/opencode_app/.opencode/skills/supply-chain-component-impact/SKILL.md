...
description: 当用户希望评估某个 CVE、组件、供应商产品或采购对象对我方产品、ECU、功能域和暴露面的影响，并输出风险优先级与整改建议时触发此技能。
---
...
- 希望评估某个 `CVE`、组件、供应商产品或采购对象是否已经影响我方平台或产品线。
- 希望识别受影响 ECU、功能域、暴露面和优先整改对象。
- 希望生成适合治理评审、快速排查或标准风险评估报告消费的供应链风险结果。

本技能默认采用“先收敛评估对象，再建立线索层，再做内部影响映射和增强解释”的分析顺序：

- `cve2oss` 负责建立漏洞和产品命中线索。
- `vehicle_iobe` 负责 ECU、暴露面、外部对端和网络关系事实。
- `ecu_func` 与 `vehicle_func` 负责功能上卷。
- `opencti`、`tara`、`ses`、`func_design_spec` 负责威胁语境、控制解释和设计语境增强。
...
- `subject_value`: 至少识别一个可用于查询或映射的入口值。
- `scope`: 至少识别 `vehicle_platform`、`product_line`、`organization_domain`、`asset_group` 中一个范围锚点。
- `focus_dimension`: 可选，允许值包括 `vulnerability`、`exposure`、`function`、`control`、`compliance`。
...
- 如果范围缺失但用户只要求快速排查，可继续执行，但必须在 `Boundary Notes` 中说明可能覆盖多个平台或产品线。
- 如果用户目标在快速排查、标准评估和深度治理之间不清晰，先追问一次，不直接进入大规模映射。
...
- 平台外的 SBOM、采购、SCA 或供应商主数据只能作为外部补充上下文，而不是平台原生事实源。
- 当缺少组件版本、产品归属或暴露面事实时，允许输出“待人工确认”的结构化报告。
...
- 当 `subject_type` 是 `cve`，优先围绕 `cve2oss` 提取产品、版本包和内部跟踪标识线索。
- 当 `subject_type` 是 `component`、`supplier`、`product` 或 `procurement_item`，先将其标准化为产品关键词、组件关键词或漏洞线索，再进入 `cve2oss` 与对象查询链。
- 若需要威胁语境增强，再补充 `opencti` 的漏洞和报告关联事实。
...
- 只把命中的漏洞、产品和跟踪标识作为线索层事实。
- 不在这一阶段输出最终风险结论。
- 若当前公开 Schema 不能提供精确版本区间，必须把结论降级为命中线索或待确认项。

## Step 4. 建立内部影响映射

围绕第一轮命中的产品、组件或版本线索，在 `vehicle_iobe` 中恢复内部影响对象：
...
- 不能只按 CVE 严重度或命中数量排序。
- 同时具备外部暴露面、关键功能影响和高质量命中线索的对象可提升为高优先级。
- 缺少版本事实或依赖弱关联的对象应降级为 `medium` 或 `low`。
