...
**定位**：外部 OpenCTI 威胁情报数据源的渐进式 STIX Schema 目录入口。
...
- **SOP Action Steps (标准作业步骤)**：明确指出依据哪几个具体的 sourceId、结合刚刚参考的 SCHEMA 规范，调用 `ai4x_query` 执行什么命令（严格遵循渐进式查询范式；`opencti` 需要时追加 `detail`）。如果有具体的 Cypher 示例，请确保符合 Schema。
